...
class GameModelViewSet(viewsets.ModelViewSet):
    ...
    @action(name="create", url_name="create", methods=['post'], detail=False, permission_classes=[permissions.AllowAny],
            authentication_classes=[])
    def create_game(self, request, **kwargs):
        return self.create(request, **kwargs)

    @action(name="join", url_name="join", methods=['post'], detail=True, permission_classes=[GamePasswordPermission],
            authentication_classes=[])
    ...
    @action(name="leave", url_name="leave", methods=['post'], detail=True, permission_classes=[PlayerInGamePermission],
            authentication_classes=[])
    ...
    @action(name="start", url_name="start", methods=['post'], detail=True, permission_classes=[GameCanStartPermission],
            authentication_classes=[])
    def start_game(self, request, pk=None):
        self.game.start()
        return Response(status=status.HTTP_200_OK)

    @action(name="end", url_name="finish", methods=['post'], detail=True, permission_classes=[GameHasStartedPermission],
            authentication_classes=[])
    def finish_game(self, request, pk=None):
        self.game.finish()
        return Response(status=status.HTTP_200_OK)

    @action(name="take-turn", url_name="take-turn", methods=['post'], detail=True,
