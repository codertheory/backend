$(function () {
    ...
    $('.comment-form').on("submit", function (event) {
        ...
        var $form = $(this),
            name = $.trim($form.find('input[name="name"]').val()),
            message = $.trim($form.find('textarea[name="message"]').val()),
            csrfToken = $.trim($form.find('input[name="csrfmiddlewaretoken"]').val()),
            url = $form.attr("action");
        ...
        // Put the results in a div
        posting.done(function () {
            $form.html('<h4>Thank you for comment! It will only be displayed once approved by a moderator</h4>').fadeTo(300, 1);
        });
